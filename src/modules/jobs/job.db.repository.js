...
module.exports = {
  ...
  findByCompanyId: async (companyId, client = prisma) => {
    const rows = await client.job.findMany({
  ...
  update: async (id, patch, client = prisma) => {
    ...
    if (patch.skills !== undefined) {
      const jobSkills = await connectOrCreateSkills(patch.skills, client);
      await client.jobSkill.deleteMany({ where: { jobId: id } });
      data.jobSkills = jobSkills;
    }
    const row = await client.job.update({
      where: { id },
      data,
      include: INCLUDE,
    });
  ...
  decrementOpenings: async (id, client = prisma) => {
